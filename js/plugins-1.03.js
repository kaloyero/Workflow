...
(function($) {
    $.fn.editable = function(e, t) {
        ...
        return this.each(function() {
            ...
            console.log("THIS",$(this), $(this).data())
            if (!$.trim($(this).html())) {
                $(this).html(n.placeholder)
            }
            $(this).bind(n.event, function(h) {
                console.log("BIDN",this,e.editing)
                ...
                var y;
                var comboEnPantalla=$(".chosen-container").length
                if (comboEnPantalla >0){
                   m=$(".chosen-container");
                    console.log("SI")
                }
                if ("cancel" == n.onblur) {
                    console.log("CANCE:::",m)
                    m.focusout(function(t) {
                          var datepickerEnPantalla=$(".datepicker").length
                          var selectEnPantalla=$(".chosen-with-drop").length
                            console.log("AVERRRR")
                                if (datepickerEnPantalla==0 && selectEnPantalla==0){
                            console.log("SEPPP CANCE:::",datepickerEnPantalla)
                            if (datepickerEnPantalla==0){
                    ...
                    if (false !== c.apply(v, [n, e])) {
                        if (false !== i.apply(v, [n, e])) {
                            if ($.isFunction(n.target)) {
                                ...
                                var seleccionoSelect=$(".chosen-container-single").length
                                console.log("SELCCIN",seleccionoSelect)
                                if (seleccionoSelect>0) {
                                    console.log("YYY",$(".chosen-container-single").prev())
                                      var valor =$(".chosen-container-single").prev().chosen().val();
            ...
            this.reset = function(t,elem) {
                var datepickerEnPantalla=$(".datepicker").length
                var cantidadActivos=$(".editable-form").length


                if (datepickerEnPantalla==0 || cantidadActivos>1){
                if (this.editing) {
                    puedeBorrar=true;
    ...
    $.editable = {
        types: {
            ...
            text: {
                ...
                date: {
                    element: function(e, t) {
                        ...
                        var n = $('<input type="text" class="form-control input-datepicker text-center"/>');
                        if (e.width != "none") {
                            n.attr("width", e.width)
                        }
                        if (e.height != "none") {
            ...
            select: {
                element: function(e, t) {
                    var n = $('<select id="prueba" name="example-c3hosen" class="select-chosen" data-placeholder="Elegi un Cliente..." style="width: 250px;"/>');
                ...
                content: function(data, settings, original) {
                    console.log("CONTENT",data,settings,original)
                    if (String == data.constructor) {
                       // eval("var json = " + data)
                    } else {
                       // var json = data
                    }
                    json=settings.loaddata
                    for (var key in json) {
                        console.log("ENTRA",json[key]["valor"])
                        ...
                        var option = $("<option/>").val(json[key]["Id"]).append(json[key]["RazonSocial"]);
                        console.log("OPTION",option)
                        $("select", this).append(option)
                    }
                    $("select", this).children().each(function() {
                        console.log("VAL",$(this).text(),data)
                        if ($(this).text() == data) {
                            console.log("PASA")
                            $(this).attr("selected", "selected")
                        }
                    });
                    $("select", this).chosen({ width:"250px" });
                    if (!settings.submit) {
                        var form = this;
